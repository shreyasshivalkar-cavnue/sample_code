...
def run():
    RedisClient = Redis(host=redis_host, port=redis_port, username="default", password=redis_auth)
    try:
        RedisClient.ping()
    ...
    f = open("radar_data_mpg_cty.txt", "wb")
    ...
    for key in stream_key:
        ...
        if streamXRead:
            ...
            for id, value in fs_data:
                ...
                # print(f"id: {id}, ts: {value[b'ts']}, payload: {value[b'payload']}")
        else:
